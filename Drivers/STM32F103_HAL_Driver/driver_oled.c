...
/*
 *  OLED_PrintFloatVal
 *  功能描述：显示带小数点的数值（整数+小数）
 *  输入参数：x --> x坐标(0~15)
 *            y --> y坐标(0~7)
 *            val     --> 要显示的浮点数
 *            decimal --> 保留几位小数（1位：0.1，2位：0.01）
 *  输出参数：无
 *  返回值：打印了多少个字符
*/
int OLED_PrintFloatVal(uint8_t x, uint8_t y, float val, uint8_t decimal)
{
    int32_t integer_part;    // 整数部分
    uint32_t decimal_part;   // 小数部分
    uint32_t multiplier = 1;
    uint8_t i;

    // 计算小数放大倍数 10/100/1000...
    for(i = 0; i < decimal; i++)
    {
        multiplier *= 10;
    }
    ...
    decimal_part = (uint32_t)((val - integer_part) * multiplier);

    // 打印整数部分
    i = OLED_PrintSignedVal(x, y, integer_part);

    // 打印小数点
    OLED_PrintString(x + i, y, ".");
    i++;

    // 打印小数部分
    OLED_PrintSignedVal(x + i, y, decimal_part);
    i += decimal;
    return i;
